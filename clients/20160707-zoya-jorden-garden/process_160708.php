...
.title{ /*text-indent:9px;*/ font-family:Verdana, Geneva, sans-serif; font-size:10px; padding: 0 5px 0 5px}
...
$imagick->setImageBackgroundColor("#333333");
//echo $image ; die;
...
$imagick->annotateImage($draw, 9, 14, 0, "castingimage");
#text
$draw->setFillColor("white");
$draw->setFillAlpha(1); 
$draw->setFontSize(24);
...
//$draw->setGravity (Imagick::GRAVITY_CENTER);
$draw->setGravity (Imagick::GRAVITY_SOUTHEAST);
$imagick->annotateImage($draw, 10, 15, 0, "castingimage");
# get ID number
...
#display on page
#extract everything after the literal "castingimage_"
preg_match("/castingimage_(.*)/", $image, $results);
$title = preg_replace('/.jpg/','',$results[1]);

echo '<li class="thumb"><a href="watermark/' . basename($image) . '" class="gallery"  title="' . $title . '" rel="preview" >
<img src="../graphics/greypixel.gif" data-original="thumbfull/' . basename($image) . '" width="150" height="150" alt="" ></a><div class="title">' . $title . '</div></li>
<noscript><img src="thumbfull/' . basename($image) . '" width="150" height="150" alt="" ></noscript>';
